...
1) How has the relative disparity for people of Asian, Black and White ethnicity changed over time?

``` {r Q1}


stop_search_2 %>% filter(ethnicity != "Other" & ethnicity != "All") %>% 
  ggplot(aes(fill=ethnicity, x=year, y=relative_disparity))+
    geom_bar(position = "dodge", stat = "identity")+
    labs(title="Stop-and-search rate over time by ethnicity",
         x="Year",
         y="Stop-and-search rate"
         )+
    theme(axis.title = element_text(size = 13),
...
stop_search_2 %>%
  ...
  ggplot(
    aes(x=reorder(geography, stops), y=stops))+
...
stop_search_2 %>% filter(ethnicity != "All" & ethnicity != "Other") %>% 
  ggplot(aes(x=legislation_type, y=stops, fill=fct_rev(ethnicity)))+
