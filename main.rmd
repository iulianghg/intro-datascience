...
nasa_temp_col_names <- c("date", "temp")
...
ggplot(data=nasa_temp, aes(x=date, y=temp))+
  ...
  labs(title="Global Yearly Land-Ocean Temperature",
       ...
       caption="Data: https://climate.nasa.gov/vital-signs/global-temperature/")
       # Set caption to specified string (link to data source)
...
sea_ice_1 <- read_excel(path=data_paths[6],
  ...
  # Gather columns 3 to 46 into key (year) and value "extent" pairs
  select(-(3:5)) %>%
  # Drop columns 3 to 5
  gather(key="key", value="month", 1) %>%
  # Gather column 1 into key and month pairs
  select(-4) %>%
  # Drop column 4 ("key")
  gather(key="day", value="day", 1) %>% 
  # Gather column 1 into key and value pairs
  select(-4) %>% 
  ...
  relocate(extent) %>%
  # Reposition the "extent" column into position 1
  arrange(month, day) %>%
...
sea_ice_2 %>% filter(year > 1979 & year < 2021) %>%
  ...
  # Set the X-axis scale type as continuous (non-discrete values)
  labs(title="Sea ice northern hemisphere",
       x="Year",
       y="Month")+
  # Change plot label names
  theme_classic()+
  # Apply theme_classic()'s settings to skip some extra steps
